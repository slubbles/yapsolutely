...
type VerifyIdentityErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function VerifyIdentityError({ error, reset }: VerifyIdentityErrorProps) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  return (
    <AuthFunnelShell
      ...
      title="The verification step hit a snag."
      description="Something interrupted the email confirmation step before the onboarding flow could continue."
      asideTitle="A broken trust step should recover fast."
      asideBody="Retrying usually fixes this route. If not, step back one screen and re-enter the funnel from the secure-account step."
    >
      <article className="rounded-[24px] border border-[color:rgba(160,91,65,0.18)] bg-[var(--surface-panel)] p-6 shadow-[var(--shadow-sm)]">
        ...
        <h2 className="mt-4 text-lg font-semibold text-[var(--text-strong)]">Unable to load verification right now</h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-body)]">
          Retry the route first. If it keeps failing, go back to the email step and continue from there.
        </p>
